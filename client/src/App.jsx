...
const LyricInput = ({ sessionId }) => {
  ...
  // User ID is now handled by parent, but we can keep a local ref if needed
  const submit = (e) => {
    e.preventDefault();
...
export default function App() {
  const [phase, setPhase] = useState("lobby"); // lobby, input, voting, generating, song_voting, winner
  ...
  // Removed unused 'username' state here
  ...
  useEffect(() => {
    ...
    socket.on("sync_state", () => {
       // Logic to sync late joiners could go here
  ...
  const joinRoom = (roomCode, user) => {
    ...
    // Removed setUsername(user) as it was setting an unused state
    socket.emit("join_room", { sessionId: roomCode, username: user });
    setPhase("input"); // Move to Phase 1
  };
